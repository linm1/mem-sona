...
// ============ 4-WAY HYBRID SEARCH (NEW ALGORITHM) ============

/**
 * Types for 4-Way Hybrid Search results.
 *
 * The new algorithm:
 * 1. Run 4 parallel searches: vectorItems, textItems, vectorNodes, textNodes
 * 2. Merge items (vector + text) with simple RRF (no weights)
 * 3. Merge nodes (vector + text) with simple RRF (no weights)
 * 4. Combine items + nodes (simple concat, no RRF)
 * 5. Apply time-decay
 * 6. Return top-K sorted by finalScore
 */
 ...
 *
 * 2. Merge items (vector + text) with simple RRF (no weights)
 *    mergedItems = mergeItemsRRF(vectorItems, textItems)
 *
 * 3. Merge nodes (vector + text) with simple RRF (no weights)
 *    mergedNodes = mergeNodesRRF(vectorNodes, textNodes)
 *
 * 4. Combine items + nodes (simple concat, no RRF)
 *    combined = combineItemsAndNodes(mergedItems, mergedNodes)
 *
 * 5. Apply time decay
 *    decayed = applyTimeDecayToResults(combined)
 *
 * 6. Return top-K sorted by finalScore
 *    return selectTopK(decayed, topK)
 *
 * This handler is defined but actual parallel search orchestration
 * happens in the action that calls the individual search functions.
 */
export const hybridSearch4WayHandler = {
  ...
  execute(
    ...
    // Step 2: Merge items (vector + text) with simple RRF
    const mergedItems = mergeItemsRRF(vectorItems, textItems);

    // Step 3: Merge nodes (vector + text) with simple RRF
    const mergedNodes = mergeNodesRRF(vectorNodes, textNodes);

    // Step 4: Combine items + nodes (simple concat)
